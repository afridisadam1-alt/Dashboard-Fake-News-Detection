...
MAX_LEN = 700  # DL sequence length
...
def load_dataset(cfg, dataset_name):
    ...
    # Detect label column
    label_col = next((c for c in ["class","label","labels"] if c in df.columns), None)
...
def load_ml_model(m, v, dataset_name):
    ...
    try:
        ...
        raise RuntimeError(f"Vectorizer for {dataset_name} not fitted properly!")
    return model, vec
...
def normalize_prediction(dataset_name, pred_label):
    ...
    if str(mapped).lower() in ["true","real","support","0"]:
        return "True"
    elif str(mapped).lower() in ["fake","1"]:
        return "Fake"
    elif str(mapped).lower() == "disinformation":
...
vc = df[label_col].value_counts().rename("Count")
st.dataframe(vc)
...
df_view = df_f.head(20)
edited = st.data_editor(df_view, hide_index=True, disabled=[c for c in df_view.columns if c!="Select"])
...
if st.button("Predict") and st.session_state.input_text:
    ...
    if is_ml:
        st.subheader("📊 Top Words (TF-IDF Importance)")
        X = vectorizer.transform([st.session_state.input_text])
        if X.nnz > 0:
